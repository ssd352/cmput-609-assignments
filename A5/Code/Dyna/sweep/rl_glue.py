...
def RLGlue(env_name, agent_name):
    ...
    """
    global _environment, _agent

    _environment = import_module(env_name)
    _agent = import_module(agent_name)
...
def RL_start():
    ...
    last_state = _environment.env_start()
    last_action = _agent.agent_start(last_state)
...
def RL_agent_start(state):
    ...
    """
    return _agent.agent_start(state)
...
def RL_agent_step(reward,state):
    ...
    int : the action taken by the agent
    """
    return _agent.agent_step(reward, state)
...
def RL_env_step(action): # returns (floating point, NumPy array, Boolean), action: NumPy array
    ...
    result = _environment.env_step(action)
...
def RL_step():
    ...
    result = _environment.env_step(last_action)
    total_reward += result['reward'];

    if result['isTerminal'] == True:
        num_episodes += 1
        _agent.agent_end(result['reward'])
        result['action'] = None
    else:
        num_steps += 1
        last_action = _agent.agent_step(result['reward'], result['state'])
        result['action'] = last_action

    return result

def RL_cleanup():
    _environment.env_cleanup()
    _agent.agent_cleanup()
...
def RL_agent_message(message):
    ...
    the_agent_response = _agent.agent_message(message_to_send)
...
def RL_env_message(message):
    ...
    the_env_response = _environment.env_message(message_to_send)
