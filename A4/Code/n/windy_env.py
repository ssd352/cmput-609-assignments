...
def env_start():
    ...
    state = (3, 0)  # The S letter in the grid
...
def env_step(action):
    ...
    # In my opinion, wind should be applied here
    # if current_state is not None:
    #     if current_state[1] in {3, 4, 5, 8}:
    #         current_state[0] -= 1
    #     elif current_state[1] in {6, 7}:
    #         current_state[0] -= 2
    ...
    reward = -1.0
    is_terminal = False
    if np.array_equal(current_state, np.array((3, 7))):
        is_terminal = True
        reward = 0.0
        current_state = None
    # print(current_state)
