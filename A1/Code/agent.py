...
class ActionPickingStrategy(object):
    def pick_action(self):
        pass
    
    def pick_first_action(self, num_actions):
        action = rand_in_range(num_actions)
        self._last_action = action
        # print('firrst action is {}'.format(self._last_action))
        return action

class EpsilonGreedyActionPickingStrategy(ActionPickingStrategy):
...
class UCBActionPickingStrategy(ActionPickingStrategy):
    def pick_action(arr):
        arg_max = np.argmax(arr)
        return arg_max
...
class constantAlphaUpdateStrategy(UpdateStrategy):
    def update(Q, step_size, reward):
        return Q + step_size * (reward - Q)

class AverageUpdateStrategy(UpdateStrategy):
    def update(Q, N, reward):
        return Q + (reward - Q) / N
...
class GenericAgent(object):
    ...
    def __init__(self, num_actions, q1, step_size):
        ...
        self._picking_strategy = None
        self._updating_strategy = None

    def pick_first_action(self):
        self._picking_strategy.pick_first_action(self._num_actions)
    ...
    def step(self, reward):
        ...
        self._Q[index] += self._step_size * (reward - self._Q[index])
        # print('_Q is {}'.format(self._Q))
...
class EpsGreedyAgent(GenericAgent):
    ...
    def __init__(self, num_actions, q1, step_size, epsilon):
        ...
        self._picking_strategy = EpsilonGreedyActionPickingStrategy()
